                        ...
                        if(checksDeleted == checksToDelete){
                          if(!deleteErrors){
                            callback(200);
                             ...
                             callback(500, {'Error' : 'Errors enconuntered while attempting to delete all of the users checks, all checks may not have been deleted from the system successfully'});
                          }
                        }
                    });
                  });
  ...
  if(id) {
    ...
    _data.read('checks',id,function(err,checkData){
      if(!err && checkData){
        // Get the token from the headers
        const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        // Verify the that the given token is valid and belongs to the user who create the token 
...
// Checks - put
// Required data: id
// Optional data: protocol, url, method, successCodes (one must be included)
                ...
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    if(checkPosition > -1) {
                      ...
                      //update users data
                      _data.update('users',checkData.userPhone,userChecks,function(err){
                        if(!err){
                          callback(200);
                        } else {
                          callback(500,{'Error' : 'Could not update the user'});
